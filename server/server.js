...
app.get('/search', function (req, res) {
	...
	globalDb.collection('inventoryLog', function(err, collection) {
	  collection.find({ "object_type" : req.param('type'), "object_id" : req.param('id'), "timestamp" : { $lt : parseInt(req.param('timestamp')) + 1} }, function(err, cursor) {
		...
		cursor.toArray(function(err,items){
           if(err || items.length == 0)
				...
				res.send({"output" : items[0].object_changes});
        });
